...
module.exports = (db) => {
      ...
      try {
        ...
        if (existingPending) {
          ...
          if (existingPending.status === "pending") {
            ...
            await pendingConnectionsCollection.deleteOne({
  ...
  router.post("/accept-request", verifyFirebaseToken, async (req, res) => {
    try {
      ...
      const notification = {
        ...
        message: `${
          receiver.fullName || receiverEmail
    ...
    async (req, res) => {
      try {
        ...
        const otherUserEmail =
          connection.user1 === userEmail ? connection.user2 : connection.user1;
        ...
        if (result.deletedCount === 0) {
          return res
            .status(404)
            .send({ message: "Connection not found or already deleted!" });
        }
    ...
    try {
      ...
      const connectionsWithDetails = await Promise.all(
        connections.map(async (connection) => {
          const otherUserEmail =
            connection.user1 === email ? connection.user2 : connection.user1;
  ...
  router.get("/get-profile", async (req, res) => {
    try {
      ...
      const profileData = await usersCollection.findOne({ email });
      if (!profileData) {
